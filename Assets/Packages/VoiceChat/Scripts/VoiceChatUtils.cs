...
namespace VoiceChat
{
    public static class VoiceChatUtils
    {
        ...
        static byte[] ZlibCompress(byte[] input, int length)
        {
            ...
            {
                using (var compressor = new Ionic.Zlib.ZlibStream(ms, CompressionMode.Compress, Ionic.Zlib.CompressionLevel.BestCompression))
                {
        ...
        static byte[] ZlibDecompress(byte[] input, int length)
        {
            using (var ms = new System.IO.MemoryStream())
            {
                using (var compressor = new Ionic.Zlib.ZlibStream(ms, CompressionMode.Decompress, Ionic.Zlib.CompressionLevel.BestCompression))
                {
                    compressor.Write(input, 0, length);
                }
